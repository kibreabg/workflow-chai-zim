...
{
    public partial class frmExpenseLiquidationApproval
        : POCBasePage,
            IExpenseLiquidationApprovalView
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (
                    _presenter.CurrentExpenseLiquidationRequest.ProgressStatus
                    == ProgressStatus.Completed.ToString()
                )
                {
                    PrintTransaction();
                }
        ...
        {
            get { return "{136836C4-1353-4DEF-A912-BF65AA84497C}"; }
        }

        private void ShowReimbersmentControls()
        {
            if (
                _presenter.CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestStatuses.Count
                    == _presenter.CurrentExpenseLiquidationRequest.CurrentLevel
                && _presenter.CurrentExpenseLiquidationRequest.CurrentApproverPosition
                    == _presenter.CurrentUser().EmployeePosition.Id
            )
            {
        ...
        private void PopProgressStatus()
        {
            ...
            {
                ddlSrchProgressStatus.Items.Add(
                    new ListItem(s[i].Replace('_', ' '), s[i].Replace('_', ' '))
                );
        ...
        private void PopApprovalStatus()
        {
            ...
            ddlApprovalStatus.Items.Add(
                new ListItem(
                    ApprovalStatus.Rejected.ToString().Replace('_', ' '),
                    ApprovalStatus.Rejected.ToString().Replace('_', ' ')
                )
            );
        }
        ...
        {
            ApprovalSetting AS = _presenter.GetApprovalSettingforProcess(
                RequestType.ExpenseLiquidation_Request.ToString().Replace('_', ' ').ToString(),
                0
            );
            string will = "";
            ...
            {
                if (
                    AL.EmployeePosition.PositionName == "Superviser/Line Manager"
                ...
                )
                {
                    ...
                    _presenter.CurrentExpenseLiquidationRequest.CurrentApprover != 0
                    && _presenter
                        .GetUser(_presenter.CurrentExpenseLiquidationRequest.CurrentApprover)
                        .EmployeePosition.PositionName == AL.EmployeePosition.PositionName
                )
                {
                    will = AL.Will;
                }
        ...
        {
            grvExpenseLiquidationRequestList.DataSource = _presenter.ListExpenseLiquidationRequests(
                txtSrchTravelAdvNo.Text,
                txtSrchRequestDate.Text,
                ddlSrchProgressStatus.SelectedValue
            );
        ...
        private void BindExpenseLiquidationRequestStatus()
        {
            ...
            foreach (
                ExpenseLiquidationRequestStatus ELRS in _presenter
                    .CurrentExpenseLiquidationRequest
                    .ExpenseLiquidationRequestStatuses
            )
            {
        ...
        private void SendEmail(ExpenseLiquidationRequestStatus ELRS)
        {
            if (ELRS.Approver != 0)
            {
                ...
                {
                    EmailSender.Send(
                        _presenter.GetUser(ELRS.Approver).Email,
                        "Expense Liquidation Approval",
                        (
                            _presenter
                                .CurrentExpenseLiquidationRequest
                            ...
                            + " Requests for Expense Liquidation to Travel Advance No. '"
                            + (
                                _presenter
                                    .CurrentExpenseLiquidationRequest
                                    .TravelAdvanceRequest
                ...
                else
                {
                    EmailSender.Send(
                        _presenter
                            .GetUser(
                                _presenter.GetAssignedJobbycurrentuser(ELRS.Approver).AssignedTo
                            )
                            .Email,
                        "Expense Liquidation Approval",
                        (
            ...
            {
                foreach (
                    AppUser Payer in _presenter.GetAppUsersByEmployeePosition(ELRS.ApproverPosition)
                )
                {
                    ...
                    else
                    {
                        EmailSender.Send(
                            _presenter
                                .GetUser(
                                    _presenter.GetAssignedJobbycurrentuser(Payer.Id).AssignedTo
                                )
                                .Email,
                            "Expense Liquidation Approval",
                            (
            ...
            )
            {
                if (
                    (
                        ...
                        || (ELRS.ApproverPosition == _presenter.CurrentUser().EmployeePosition.Id)
                        || _presenter.CurrentUser().Id
                            == (
                                _presenter.GetAssignedJobbycurrentuser(ELRS.Approver) != null
                                    ? _presenter
                                        .GetAssignedJobbycurrentuser(ELRS.Approver)
                                        .AssignedTo
                                    : 0
                            )
                    )
                ...
                )
                {
                    ...
                    ELRS.AssignedBy =
                        _presenter.GetAssignedJobbycurrentuser(ELRS.Approver) != null
                            ? _presenter.GetAssignedJobbycurrentuser(ELRS.Approver).AppUser.FullName
                            : "";
                    if (ELRS.ApprovalStatus != ApprovalStatus.Rejected.ToString())
                    {
                        _presenter.CurrentExpenseLiquidationRequest.ProgressStatus =
                            ProgressStatus.Completed.ToString();
                        _presenter.CurrentExpenseLiquidationRequest.ExpenseReimbersmentType =
                            ddlType.SelectedValue;
                        _presenter.CurrentExpenseLiquidationRequest.ReimbersmentNo = txtNumber.Text;
                        _presenter
                            .CurrentExpenseLiquidationRequest
                            .TravelAdvanceRequest
                            .ExpenseLiquidationStatus = "Finished";
                        GetNextApprover();
                        ELRS.Approver = _presenter.CurrentUser().Id;
                        _presenter.CurrentExpenseLiquidationRequest.CurrentStatus =
                            ELRS.ApprovalStatus;
                        Log.Info(
                            _presenter.GetUser(ELRS.Approver).FullName
                                + " has "
                                + ELRS.ApprovalStatus
                                + " Expense Liquidation Request made by "
                                + _presenter
                    ...
                    else
                    {
                        ...
                        _presenter
                            .CurrentExpenseLiquidationRequest
                            .TravelAdvanceRequest
                            .ExpenseLiquidationStatus = "Completed";
                        Log.Info(
                            _presenter.GetUser(ELRS.Approver).FullName
                                + " has "
                                + (ELRS.ApprovalStatus).ToUpper()
                                + " Expense Liquidation Request made by "
                                + (
                                    _presenter
                                        .CurrentExpenseLiquidationRequest
        ...
        private void DeleteLiquidationIfRejected()
        {
            _presenter
                ...
                .TravelAdvanceRequest
                .ExpenseLiquidationStatus = "Completed";
            IList<ExpenseLiquidationRequestDetail> detailList =
                _presenter.CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestDetails.ToList();
            foreach (ExpenseLiquidationRequestDetail detail in detailList)
            {
                _presenter.CurrentExpenseLiquidationRequest.RemoveExpenseLiquidationRequestDetail(
                    detail.Id
                );
            }
            ...
            _presenter.SaveOrUpdateExpenseLiquidationRequest(
                _presenter.CurrentExpenseLiquidationRequest
            );
        ...
        protected void grvExpenseLiquidationRequestList_SelectedIndexChanged(
            object sender,
            EventArgs e
        )
        {
        ...
        protected void grvExpenseLiquidationRequestList_RowCommand(
            Object sender,
        ...
        )
        {
            ...
            {
                liqID = (int)
                    grvExpenseLiquidationRequestList
                        .DataKeys[Convert.ToInt32(e.CommandArgument)]
                        .Value;
                Session["ReqID"] = liqID;
                _presenter.CurrentExpenseLiquidationRequest =
                    _presenter.GetExpenseLiquidationRequest(liqID);
                if (e.CommandName == "ViewItem")
                {
                    //_presenter.OnViewLoaded();
                    dgLiquidationRequestDetail.DataSource = _presenter
                        .CurrentExpenseLiquidationRequest
                        .ExpenseLiquidationRequestDetails;
        ...
        protected void grvExpenseLiquidationRequestList_PageIndexChanging(
            object sender,
        ...
        {
            if (
                _presenter.CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestStatuses
                    != null
                && e.Row.RowType == DataControlRowType.DataRow
                && _presenter
                    .CurrentExpenseLiquidationRequest
                    .ExpenseLiquidationRequestStatuses[e.Row.RowIndex]
                    .Approver != 0
            )
            {
        ...
        protected void ddlEdtAccountDescription_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtAccountCode.Text = _presenter
                .GetItemAccount(Convert.ToInt32(ddl.SelectedValue))
                .AccountCode;
        }
            ...
            try
            {
                ...
                )
                {
                    ...
                    if (ddlApprovalStatus.SelectedValue != "Rejected")
                    {
                        Master.ShowMessage(
                            new AppMessage(
                                "Expense Liquidation Approval Processed",
                                RMessageType.Info
                            )
                        );
                    }
                    else
                    {
                        Master.ShowMessage(
                            new AppMessage(
                                "Expense Liquidation Approval Rejected",
                                RMessageType.Info
                            )
                        );
                    }
        ...
        private void PrintTransaction()
        {
            ...
            lblRequestedDateResult.Text =
                _presenter.CurrentExpenseLiquidationRequest.RequestDate.Value.ToShortDateString();
            if (_presenter.CurrentExpenseLiquidationRequest.TravelAdvRequestDate != null)
                lblTravelAdvReqDateResult.Text =
                    _presenter.CurrentExpenseLiquidationRequest.TravelAdvRequestDate.Value.ToShortDateString();
            ...
            lblPurposeofAdvanceResult.Text =
                _presenter.CurrentExpenseLiquidationRequest.Comment.ToString();
            lblApprovalStatusResult.Text =
                _presenter.CurrentExpenseLiquidationRequest.ProgressStatus.ToString();
        ...
        protected void dgLiquidationRequestDetail_ItemDataBound(
            object sender,
            DataGridItemEventArgs e
        )
        {
            if (
                _presenter.CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestDetails != null
            )
            {
                ...
                if (ddlProject != null)
                {
                    ...
                    if (
                        _presenter
                            .CurrentExpenseLiquidationRequest
                            .ExpenseLiquidationRequestDetails[e.Item.DataSetIndex]
                            .Project
                            .Id != 0
                    )
                    {
                        ListItem liI = ddlProject.Items.FindByValue(
                            _presenter
                                .CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestDetails[
                                    e.Item.DataSetIndex
                                ]
                                .Project.Id.ToString()
                        );
                ...
                if (ddlAccountDescription != null)
                {
                    ...
                    {
                        ListItem liI = ddlAccountDescription.Items.FindByValue(
                            _presenter
                                .CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestDetails[
                                    e.Item.DataSetIndex
                                ]
                                .ItemAccount.Id.ToString()
                        );
                        if (liI != null)
                            liI.Selected = true;
                    }
                ...
                {
                    foreach (
                        ExpenseLiquidationRequestDetail detail in _presenter
                            .CurrentExpenseLiquidationRequest
                            .ExpenseLiquidationRequestDetails
                    )
                    {
                ...
                if (e.Item.ItemType == ListItemType.Footer)
                {
                    ...
                    {
                        _totalActualExpenditure =
                            _totalActualExpenditure + detail.ActualExpenditure;
                    }
        ...
        protected void dgLiquidationRequestDetail_EditCommand(
            object source,
        ...
        protected void dgLiquidationRequestDetail_UpdateCommand(
            object source,
            DataGridCommandEventArgs e
        )
        {
            ...
            if (CPRDId > 0)
                cprd =
                    _presenter.CurrentExpenseLiquidationRequest.GetExpenseLiquidationRequestDetail(
                        CPRDId
                    );
            else
                cprd = (ExpenseLiquidationRequestDetail)
                    _presenter.CurrentExpenseLiquidationRequest.ExpenseLiquidationRequestDetails[
                        e.Item.ItemIndex
                    ];
            ...
            try
            {
                ...
                DropDownList ddlAccountDescription =
                    e.Item.FindControl("ddlEdtAccountDescription") as DropDownList;
                cprd.ItemAccount = _presenter.GetItemAccount(
                ...
                Master.ShowMessage(
                    new AppMessage(
                        "Expense Liquidation Detail Successfully Updated",
                        RMessageType.Info
                    )
                );
            }
            catch (Exception ex)
            {
                Master.ShowMessage(
                    new AppMessage(
                        "Error: Unable to Update Expense Liquidation Detail. " + ex.Message,
                        RMessageType.Error
                    )
                );
                ExceptionUtility.LogException(ex, ex.Source);
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt1 = _presenter
                    .ExportTravelAdvance(_presenter.CurrentExpenseLiquidationRequest.Id)
                    .Tables[0];
                ...
                using (ExcelPackage pck = new ExcelPackage())
                {
                    ...
                    Response.AddHeader(
                        "content-disposition",
                        "attachment;  filename=Journal Entry - '"
                            + _presenter
                                .CurrentExpenseLiquidationRequest
                                .TravelAdvanceRequest
                                .TravelAdvanceNo
